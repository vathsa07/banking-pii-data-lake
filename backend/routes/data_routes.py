...
def get_gold_data(payload: dict = Depends(decode_token)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch Gold data: {e!s}")
...
def get_transaction_summary(payload: dict = Depends(decode_token)):
    ...
    try:
        gold_obj = s3.get_object(
            Bucket="gold", Key="transaction_summary.parquet")
        ...
        export_path = os.path.abspath(
            os.path.join(
                ...
                "../../powerbi_export/transaction_summary.parquet",
            )
        )
...
def get_risk_summary(payload: dict = Depends(decode_token)):
    ...
    try:
        gold_obj = s3.get_object(
            Bucket="gold", Key="risk_segment_summary.parquet")
        df = pd.read_parquet(pd.io.common.BytesIO(gold_obj["Body"].read()))
        return {"status": "success", "data": df.to_dict(orient="records")}
    except Exception:
        export_path = os.path.abspath(
            os.path.join(
                os.path.dirname(__file__),
                "../../powerbi_export/risk_segment_summary.parquet",
            )
        )
        if os.path.exists(export_path):
            df = pd.read_parquet(export_path)
