...
from backend.auth import require_roles, decode_token
...
def get_pipeline_status(payload: dict = Depends(decode_token)):
    ...
    try:
        ...
        return {
            "status": "active_standalone",
            "dag_id": "banking_medallion_pipeline",
            "message": f"Airflow status check: {str(e)}",
            "latest_run": {
...
def get_data_quality_results(payload: dict = Depends(require_roles(["admin", "compliance_officer"]))):
    """
    ...
    try:
        ...
        invalid_names = df_gold[~df_gold["masked_name"].astype(str).str.startswith("TOK_")]
        ...
        checks.append({
            ...
            "details": "Zero raw emails found in Gold layer." if not has_email_leak else f"LEAK DETECTED: {len(leaked_emails)} raw emails found!",
            "severity": "CRITICAL"
        })

        # Check 4: Schema integrity check
        required_cols = ["customer_id", "masked_name", "masked_email", "state", "risk_segment", "total_balance"]
        ...
        checks.append({
            ...
            "details": "All required gold columns present." if not missing_cols else f"Missing columns: {missing_cols}",
            "severity": "HIGH"
        })
        ...
        checks.append({
            ...
            "status": "FAIL",
            "details": f"Failed to access Gold S3 layer: {str(e)}",
            "severity": "CRITICAL"
        })
...
@router.post("/trigger")
def trigger_pipeline(payload: dict = Depends(require_roles(["admin", "compliance_officer", "analyst"]))):
    """
    ...
    try:
        ...
        return {"status": "success", "message": "Pipeline triggered and completed successfully."}
    except Exception as e:
